...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnExport) {
        btnExport.addEventListener('click', () => {
            ...
            const antiVisionVal = (inputAntiVision && inputAntiVision.value.trim()) || '（未填写）';
            const visionVal = (inputVision && inputVision.value.trim()) || '（未填写）';
            const fearVal = (inputFear && inputFear.value.trim()) || '（未填写）';
            const h1Val = (habit1 && habit1.value.trim()) || '（未填写）';
            const h2Val = (habit2 && habit2.value.trim()) || '（未填写）';
            const h3Val = (habit3 && habit3.value.trim()) || '（未填写）';
